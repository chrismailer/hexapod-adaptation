...
for col, niche_size in enumerate(niches):
    for map_n in range(1, n_maps+1):
        fits, descs, x = load_map(f'./niches_{niche_size}/map_{map_n}.dat')
        global_performance[map_n-1, col] = max(fits) / 0.5
        coverage[map_n-1, col] = fits[fits > 0].shape[0] / niche_size
        print('.', end='')
...
np.savetxt('global_performance.dat', global_performance)
np.savetxt('global_reliability.dat', global_reliability)
np.savetxt('precision.dat', precision)
np.savetxt('coverage.dat', coverage)
